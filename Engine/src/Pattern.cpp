...
namespace Pattern
{
	...
	uint64_t LookupPattern(const std::string& PatternName)
	{
		...
		else
		{
			...
			if (Configurations.Has(s))
				return std::stoull(Configurations.Get(s), 0, 16);
			else
			{
				// TODO: Make this work again!
				//if (Configurations.Verbosity > -2)
	...
	std::vector<std::string> GetActivePattern()
	{
		...
		std::string PatternString = Configurations.Has("active pattern") ? Configurations.Get("active pattern") : "";
	...
	void Initialize(const bool weights, const bool singleton)
	{
		...
		if (singleton)
		{
			...
			if (weights)
			{
				...
				WeightsPath = Configurations.Get("ProgramFolderPath") + Configurations.Get("WeightsFolderPath");
			}
...
int EvaluatePattern(const uint64_t P, const uint64_t O, const uint64_t emptyCount)
{
	...
	return std::clamp(RoundInt(score), -64, 64);
}
...
namespace CountLastFlip
{
	...
	uint8_t flip_loop_dir(const uint8_t P, const uint8_t O, const int move, const int dX)
	{
		uint8_t flips = 0;
		...
		{
			uint8_t tmp = 1 << i; // The bit to look at
	...
	uint8_t CountFlip(const uint8_t P, const uint8_t move)
	{
		if (P & (1 << move)) return 0;
		const uint8_t O = P ^ 0xFF ^ (1 << move);
		return PopCount(flip_loop_dir(P, O, move, +1) | flip_loop_dir(P, O, move, -1));
	}

	void Initialize()
	{
		for (int i = 0; i < 256; i++) CLF_0[i] = 2 * CountFlip(i, 0);
		for (int i = 0; i < 256; i++) CLF_1[i] = 2 * CountFlip(i, 1);
		for (int i = 0; i < 256; i++) CLF_2[i] = 2 * CountFlip(i, 2);
		for (int i = 0; i < 256; i++) CLF_3[i] = 2 * CountFlip(i, 3);
		for (int i = 0; i < 256; i++) CLF_4[i] = 2 * CountFlip(i, 4);
		for (int i = 0; i < 256; i++) CLF_5[i] = 2 * CountFlip(i, 5);
		for (int i = 0; i < 256; i++) CLF_6[i] = 2 * CountFlip(i, 6);
		for (int i = 0; i < 256; i++) CLF_7[i] = 2 * CountFlip(i, 7);
	}
	...
	uint8_t CLF_F4(const uint64_t P) { return CLF_5[BExtr(P, 24, 8)] + CLF_3[PExt(P, 0x2020202020202020ULL)] + CLF_3[PExt(P, 0x0000804020100804ULL)] + CLF_2[PExt(P, 0x0204081020408000ULL)]; }
	...
	uint8_t CLF_pass(const uint64_t P) { return 0; }


	uint8_t (*CLF_fp[65])(const uint64_t) = {
		...
		CLF_A8, CLF_B8, CLF_C8, CLF_D8,	CLF_E8, CLF_F8, CLF_G8, CLF_H8, CLF_pass
	};
}

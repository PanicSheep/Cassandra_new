...
template <typename T>
long double Average(const std::vector<T>& vec)
{
	long double E_of_X = 0;
	std::size_t N = 0;
...
{
	inline void add(const T& value) // non thread-safe
	{
		_N++;
...
public:
	...
	CRunningStatistic(const std::vector<T> vec) : CRunningStatistic()
	{
		if (vec.size()) {
			_min = vec[0];
			_max = vec[0];
	...
	void Add(const T& value) // thread-safe
	{
	...
	inline long double sigma() const { return std::sqrt(Var()); }
	inline long double StandardDeviation() const { return sigma(); }

	inline long double deltasigma() const { return std::sqrt(Var()) / std::sqrt(2.0 * (static_cast<long double>(_N) + 1.0)); }
	...
	{
		CRunningStatistic<T> res = CRunningStatistic<T>();

...
TEST(Utility, FlipDiagonal) {
	for (int i = 0; i < 8; i++)
		for (int j = 0; j < 8; j++)
			ASSERT_EQ(FlipDiagonal(MakeBit(i * 8 + j)), MakeBit(j * 8 + i));
}

TEST(Utility, FlipCodiagonal) {
	for (int i = 0; i < 8; i++)
		for (int j = 0; j < 8; j++)
			ASSERT_EQ(FlipCodiagonal(MakeBit(i * 8 + j)), MakeBit(63 - (j * 8 + i)));
}

TEST(Utility, FlipVertical) {
	for (int i = 0; i < 8; i++)
		for (int j = 0; j < 8; j++)
			ASSERT_EQ(FlipVertical(MakeBit(i * 8 + j)), MakeBit((7 - i) * 8 + j));
}

TEST(Utility, FlipHorizontal) {
	for (int i = 0; i < 8; i++)
		for (int j = 0; j < 8; j++)
			ASSERT_EQ(FlipHorizontal(MakeBit(i * 8 + j)), MakeBit(i * 8 + (7 - j)));
}
...
void TestFlip(const CMove move)
{
	...
	{
		CPosition pos = MakeRandomPosition(~(MakeBit(move)));
		ASSERT_EQ(Flip(pos, move), Flip_loop(pos, move));
	}
}
...
TEST(FlipLoop, OneFlip) {
	const CPosition pos(0xFF00000000000000ui64, 0x00FFFFFFFFFFFF7Eui64);
	const CMove move(A1);
	const uint64_t flip = Flip_loop(pos, move);

	ASSERT_EQ(pos.GetP() & flip, 0ui64);
	ASSERT_EQ(pos.GetO() & flip, flip);
	ASSERT_EQ(flip, 0x0041211109050300ui64);
}
...
TEST(MacrosHell, GetBitTest) {
	uint64_t a = 0x8000000000000001ui64;
...
{
	ASSERT_EQ(CPosition::StartPosition().PossibleMoves(), CMoves(0x0000102004080000ui64));
}

TEST(PossibleMoves, PossibleMoves_StartPositionETH)
{
	ASSERT_EQ(CPosition::StartPositionETH().PossibleMoves(), CMoves(0x00000000003C0000ui64));
}
